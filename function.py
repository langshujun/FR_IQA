...
def load_data(mod, dataset, worker_idx):   #worker_idx表示的是失真类型
    ...
    if dataset == 'CSIQ':
        data_dir = os.path.join('csv')
        worker_orignal = pd.read_csv(os.path.join(data_dir, 'CSIQ.csv'), sep=',')  #阅读csv文件
        dis_image_path = 'data/CSIQ/dst_imgs/'
        ref_image_path = 'data/CSIQ/src_imgs/'

    if dataset == 'KADID10k':
        data_dir = os.path.join('csv')
        worker_orignal = pd.read_csv(os.path.join(data_dir, 'kadid10k.csv'), sep=',')  #阅读csv文件
        dis_image_path = 'data/KADID10k/images/'
        ref_image_path = 'data/KADID10k/images'
